...
### Added
- **Declined Work Recovery — date scoping, junk filtering, and win-back outreach drafts.** Two upgrades to `/intel/recovery`, plus a detection→outreach loop that DRAFTS and LOGS win-back texts. **Live customer SMS is scaffolded but OFF by default** — it requires `OUTREACH_SEND_ENABLED=true` plus Twilio creds and a human in the loop. Nothing is auto-sent.
  - **Better transcript selection:** `lib/recovery.ts` now filters junk before the model pass — obvious test/ambient rigs are dropped by filename (`test`/`gaming`/`sample`/`ambient`/`mic-check`/…) before spending an S3 read, and low-signal clips are dropped by content (a hard `MIN_WORDS=15` empty floor plus a conservative `SIGNAL_MIN_WORDS=25` ambient floor and a repeated-token guard). Most-recent mode now over-fetches a wider pool (`RECENT_POOL=60`) and keeps the newest 12 *real* conversations, fixing the old bug where a flood of short ambient clips crowded real work out of the top-12 and the page read empty.
  - **Date scoping:** `getRecovery(advisorId?, day?, force?)` and the transcript loader accept an optional `day` (`YYYY-MM-DD`); when given, transcripts are selected by S3 `LastModified` falling on that UTC day (mirrors the audit lambda), newest-first, capped at 40/day. No `day` = unchanged most-recent behavior. `day` is part of the cache key. `RecoveryResult` gains a `day` field. `GET /api/intel/recovery` accepts `?day=YYYY-MM-DD` (validated).
  - **UI date picker:** `components/RecoveryDatePicker.tsx` — a dark-theme `<input type="date">` that reloads with `?day=…` (capped at today, UTC) plus a "Most recent" reset; the header shows the active mode ("most recent" vs a specific day). The page reads `searchParams.day`.
  - **Outreach loop (draft + log):** `lib/outreach.ts` — `draftOutreach(item)` writes a short, warm, ready-to-send win-back SMS via Claude **Haiku** (cheap), bounded to ≤320 chars with no placeholders; `logOutreach({advisorId, item, draftText, phone?})` PutItems to the `servicesync-recovery-outreach` table (PK `advisor_id`, SK `ts`) with `status:"drafted"`; `listOutreach(advisorId)` returns recent records newest-first. `app/api/intel/recovery/outreach/route.ts` — `POST {advisorId?, item, phone?}` drafts + logs and returns the record (Node runtime, try/catch → 500). On the page each declined item gets a **"Draft win-back text"** button (`components/DraftWinBackButton.tsx`) that shows the draft with a copy affordance and a "saved to outreach log" note, and a new **Outreach log** section lists recent drafts + status.
  - **SMS send — scaffold, DEFAULT OFF, not wired live:** `lib/sms.ts` — `sendOutreachSms(phone, text)` is a NO-OP returning `{sent:false, reason:'sending disabled'}` unless `OUTREACH_SEND_ENABLED==='true'` AND `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_FROM_NUMBER` are all set. Twilio is **lazy-imported only inside the enabled branch**, so the build never depends on it and no send path runs by default. The UI shows send as "not enabled". **Nothing auto-sends.**
  - **Deploy note:** the dashboard's AWS identity needs `dynamodb:PutItem` + `dynamodb:Query` on the `servicesync-recovery-outreach` table (already granted RW). Optional table-name override via `TABLE_OUTREACH`.
- **CI build gate** (`.github/workflows/ci.yml`) — GitHub Actions workflow that runs on every `pull_request` and on `push` to `master`. Ubuntu + Node 22, `npm ci` → `npm run lint` → `npm run build` (with `DASHBOARD_PASSWORD: x` so the fail-closed middleware build step doesn't error). No deploy. Closes the gap where merges were landing without an automated build/lint check.
- **Conversations** (`/intel/conversations`) — review the advisor↔assistant chat history. Reads the `servicesync-conversations` DynamoDB table (name from `TABLE_CONVERSATIONS`, PK `advisor_id` / SK `ts` ISO-8601), returning an advisor's most recent exchanges newest-first. Each exchange renders as a chat log: an advisor message bubble (user) then an assistant reply bubble, with a timestamp and a small `model · in/out tok` caption. Advisor-aware: scopes to the selected advisor (`ss_advisor` cookie) or the first advisor with any recorded conversation; a page-local selector lists advisors sourced from the conversations table itself, so it can offer advisors that have chat history even if they aren't in the `servicesync-advisors` directory. Includes a prominent privacy note ("Advisor↔assistant conversations are stored and auto-delete after 90 days" — the table carries a `ttl` DynamoDB TTL). Empty state ("No conversations recorded yet.") and a graceful "unavailable" card if the table read fails (mirrors the Recovery page).
  - `lib/conversations.ts` — `listConversations(advisorId, limit=100)` runs a `Query` (`ScanIndexForward:false`) via the shared doc client from `lib/tracker/dynamo.ts`; `listConversationAdvisors()` does a bounded, paginated `Scan` projecting only `advisor_id`, deduped and sorted.
  - `app/api/intel/conversations/route.ts` — `GET ?advisor=<id>` returns that advisor's exchanges; without `advisor` it returns `{ advisors: string[] }` (Node runtime, force-dynamic, intel-style `Cache-Control`, try/catch → 500).
  - `app/intel/conversations/page.tsx` — server component (Node runtime, force-dynamic) rendering the chat log + privacy note.
  - `components/ConversationAdvisorSelector.tsx` — client `<select>` that sets the `ss_advisor` cookie and refreshes.
  - `components/Sidebar.tsx` — "Conversations" nav item (💬) after Audits.
  - **Deploy note:** the dashboard's AWS identity needs `dynamodb:Query` + `dynamodb:Scan` on the `servicesync-conversations` table (already granted).
- **Cloud spend — month to date** on `/intel/usage`. An additive owner-side section above the per-advisor tables showing the dashboard's own AWS bill for the current calendar month: a prominent total plus a small table of the top ~8 services by `UnblendedCost` (non-zero, sorted desc). Independent of the per-advisor usage report, so it renders even if that read fails.
  - `lib/awscost.ts` — `getCloudSpendMTD()` calls Cost Explorer `GetCostAndUsage` (MONTHLY, `UnblendedCost`, grouped by SERVICE) for first-of-month → today+1 (UTC), via a `CostExplorerClient` singleton pinned to `us-east-1` (Cost Explorer's endpoint). Wrapped in try/catch → `{ available: false, error }` so an AccessDenied (missing `ce:GetCostAndUsage`) degrades to a muted fallback card instead of crashing.
  - `app/intel/usage/page.tsx` — `CloudSpendCard` (cyan accent, matching the branding tokens). Notes on the card that AWS credits may mask real spend (figures can read ~$0 until credits are exhausted), and renders a muted "needs `ce:GetCostAndUsage`" card when unavailable. Existing per-advisor content is unchanged.
  - **Deploy note:** adds the `@aws-sdk/client-cost-explorer` dependency (run `npm install` on deploy) and the dashboard's AWS identity needs `ce:GetCostAndUsage`.
